...
- **意义**：
  ...
  - 为后续RTL设计提供参考数据。
    ...
    <img src="e1e3cfe168bd64e2f9fcd5208f2751fd.png" alt="90cc41fc-f897-4edf-b780-e40fbdf208e7.png" style="zoom:50%;" />

### 2、FPGA总体架构设计

![系统架构.drawio.png](eb00da0dd0ee89c7927c82c4765e7842.png)
...
![a7f911025a82f04ff41c056dfe5f3951.png](a04f89b6aa95404e4bd7034cdab75716.png)
...
![截图](d081e094c77bbf487d321169399bc0da.png)

- 可跑**800MHz**（测试板上限，理论最高近1GHz），每**1.25ns**可以处理一轮64位的数据。

### 2. 路径裕量

![e57b2ff3-d9cf-484e-a305-5d117ce51fdd.png](111a94c744da7a2f2c04bfc8d9079f84.png)
...
<img src="b5991070a10f2f3c954448b081e9d4a4.png" alt="0ef503eabb71af6c1de6412ecafd582a.png" style="zoom:40%;" />
...
- 在 800MHz 时钟频率下，IP 核的动态功耗约为 1194mW。其中，DES 加解密流水线模块是动态功耗的主要消耗者，由于该模块包含 16 轮 Feistel 结构的迭代运算，涉及大量的异或、置换、S 盒替换等操作，信号翻转频繁，导致其动态功耗占比超过 60%；密钥调度模块因需要进行多次置换和移位操作，动态功耗占比约 20%；其他辅助模块的动态功耗占比约 20%。
  ...
  ![1745dee8-3e83-471f-a1a7-84c52d726ad6.png](c0d31f4f55116954715e7a3a117dbdce.png)
...
![1dc472eacb1adf2680318b41c654662e.png](c2c6cd5d1c7944c123dd8e580324c9d6.png)
...
- 密钥置换盒 PC-1/PC-2 测试：覆盖 “64 位密钥→56 位有效密钥→16 轮子密钥” 的全流程，验证密钥调度的每一步位操作是否符合规范。![4fefcbf63cb2dfdcc04d72c6c5644e12.png](4fefcbf63cb2dfdcc04d72c6c5644e12.png)
...
- 明文输入（`text[63:0]`）：初始输入为十六进制字符串0123456789abcdef，后续出现31323333...、00000000...、e957442...、ffffffff...等多组数据，text_valid信号为 1 表示这些明文数据有效。
  ...
  <img src="4d05566c1fb496bc6f22750c336dd1a0.png" alt="61f939cff638be91b1d8f46468ae0255.png" style="zoom:40%;" />
- 从该仿真波形图中，DES 模块在加密模式下明文0123456789abcdef、密钥3132333435363738输出的密文为 **53DB7DA3F194A15D**（十六进制，64 位）结果正确。![72209f14f447f333a52e963c693f119b.png](0e8dbee747af5f52d089e0cea39fc195.png)
  
  ![7e6f1a7fc19ec2b11b8f0c29fe8a7eb9.png](8df162bbd52850f20f17498f6a88f105.png)
- 复位信号（reset_n）：当前为 0，处于复位状态，text[63:0]、key[63:0]虽有数据值，但输出结果仍会正常清零。<img src="792f102ed3ca72eddf5ee5da8b232b7e.png" alt="f37323bcfc2581dde95b0a62b41a11a5.png" style="zoom:40%;" />
...
- 通过搭建串口通信仿真模型，模拟上位机与 FPGA 之间的串口数据传输。将 IP 核的加解密结果通过串口发送模块发送，同时接收上位机发送的明文或密文数据，验证串口数据传输的正确性以及 IP 核与串口模块接口的兼容性。仿真重点关注波特率匹配、数据帧格式（起始位、数据位、校验位、停止位）以及数据传输的完整性。![截图](9a963d1581adbde97e729a6e47daeb71.png)
...
- 串口仿真中，设置波特率为 9600bps，数据格式为 8 位数据位、1 位停止位、无校验位。上位机发送的明文数据经 FPGA 接收后，通过 IP 核加密处理，再由串口发送回上位机，上位机接收到的密文与预期一致；同样，上位机发送的密文经 FPGA 接收解密后，发送回上位机的明文准确无误。数据传输过程中无丢包、错包现象，验证了串口通信的可靠性。
  ...
  ![1feb2345be12b77263612347ece25126.png](75cfd5320b85555e3cbcb38058d9aba3.png)
...
<img src="cbf1f85c526da359ca0834e91ad7732d.png" alt="截图" style="zoom:50%;" />
